namespace Trine.Mobile.Components.Navigation
{
    public static class NavigationParameterKeys
    {
        public static readonly string _Contract = "Contract";
        public static readonly string _NavigatedFromUri = "NavigatedFromUri";
        public static readonly string _IsUserPickerModeEnabled = "IsUserPickerModeEnabled";
        public static readonly string _User = "User";
        public static readonly string _Organization = "Organization";
        public static readonly string _Mission = "Mission";
        public static readonly string _OrganizationId = "OrganizationId";
        public static readonly string _CreateMissionRequest = "CreateMissionRequest";
        public static readonly string _Activity = "Activity";
        public static readonly string _IsActivitySigned = "IsActivitySigned";
        public static readonly string _IsActivityRefused = "IsActivityRefused";
        public static readonly string _ActivityComment = "ActivityComment";
        public static readonly string _Absence = "Absence";
        public static readonly string _Period = "Period";
        public static readonly string _IsLaterTextVisible = "IsLaterTextVisible";
        public static readonly string _Bytes = "Bytes";

    }
}
